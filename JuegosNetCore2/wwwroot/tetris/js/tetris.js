...
class Game {
    ...
    static TIMEOUT_LOCK_PUT_NEXT_PIECE = 1;
...
`);
    ...
    initControls() {
        ...
        this.$btnDown.addEventListener("click", () => {
            ...
            this.attemptMoveDown('App');
        });
        this.$btnRight.addEventListener("click", () => {
            if (!this.canPlay) return;
            this.attemptMoveRight('App');
        });
        this.$btnLeft.addEventListener("click", () => {
            if (!this.canPlay) return;
            this.attemptMoveLeft('App');
        });
        this.$btnRotate.addEventListener("click", () => {
            if (!this.canPlay) return;
            this.attemptRotate('App');
        });
    ...
    addScore(rows) {
        ...
        let speed = this.lines === 3 ? 800 : this.lines === 5 ? 700 : this.lines === 10 ? 500 : this.lines >= 12 ? 300 : 0;
        if (speed > 0) {
            Game.PIECE_SPEED = speed;
            this.intervalId = setInterval(this.mainLoop.bind(this), Game.PIECE_SPEED);
        }
    ...
    mainLoop() {
        ...
        if (this.figureCanMoveDown()) {
            ...
            setTimeout(() => {
                ...
                if (this.playerLoses()) {
                    Swal.fire("Juego terminado", `Puntaje: ${this.score} Lineas: ${this.lines} <br>`);

                    this.$btnDown.remove();
                    this.$btnRight.remove();
                    this.$btnLeft.remove();
                    this.$btnRotate.remove();
    ...
    refreshScore() {
        this.$score.textContent = `Puntaje: ${this.score} Lineas: ${this.lines}`;
    }

    initSounds() {
        this.sounds.background = Utils.loadSound("/tetris/assets/New Donk City_ Daytime 8 Bit.mp3", true);
        this.sounds.success = Utils.loadSound("/tetris/assets/success.wav");
        this.sounds.denied = Utils.loadSound("/tetris/assets/denied.wav");
        this.sounds.tap = Utils.loadSound("/tetris/assets/tap.wav");
    }
